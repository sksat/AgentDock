# AgentDock - 設計ドキュメント

## 概要

AgentDock は、Claude Code CLI をサーバーサイドで実行し、フロントエンドのリッチ UI から操作できるようにするプロジェクトです。単なるターミナルエミュレーションではなく、**plan モード、パーミッションリクエスト、選択肢、差分提案などを専用 UI で表示**します。

## 要件

- **フロントエンド**: React + TypeScript + shadcn/ui
- **認証**: 不要（自分専用）
- **セッション永続化**: 必要（Claude Code のセッション機能を使用）
- **リッチUI**: ターミナル出力だけでなく、構造化されたイベント（パーミッション、質問、差分）を専用 UI で表示
- **Claude Max 互換**: 公式にサポートされた方法のみ使用

## 背景知識

### Claude Code CLI Headless Mode

Claude Max ライセンスで公式にサポートされた方法。

```bash
# 基本的な使い方
claude -p "prompt" --output-format stream-json

# セッション継続
claude -p "follow-up" --continue
claude -p "specific session" --resume <session_id>

# パーミッションプロンプトを MCP ツールに委譲
claude -p "prompt" --permission-prompt-tool mcp__bridge__permission_prompt
```

### --permission-prompt-tool オプション

パーミッションリクエストを MCP ツールに委譲する公式機能。

**動作メカニズム** (3層チェック):
1. `--allowedTools` / `--disallowedTools` で事前定義ルールを確認
2. ルールがマッチすれば即座に許可/拒否
3. ルールなしなら MCP ツールを呼び出し

**MCP ツールのレスポンスフォーマット**:
```json
// 許可
{"behavior": "allow", "updatedInput": {...}}

// 拒否
{"behavior": "deny", "message": "理由"}
```

### セッション管理

- セッションは `~/.claude/sessions/` に自動保存
- `--resume <session_id>` で特定のセッションを再開
- `--continue` で最新のセッションを継続
- stream-json 出力に `session_id` が含まれる

---

## アーキテクチャ

```
┌─────────────────────────────────────────────────────────────┐
│                     Server (Node.js)                         │
│  ┌────────────────────────────────────────────────────────┐ │
│  │                    Hono + WebSocket                     │ │
│  │  ┌──────────────────────────────────────────────────┐  │ │
│  │  │              Session Manager                      │  │ │
│  │  │  ┌────────────────┐  ┌────────────────┐          │  │ │
│  │  │  │   Session #1   │  │   Session #2   │  ...     │  │ │
│  │  │  │ ┌────────────┐ │  │ ┌────────────┐ │          │  │ │
│  │  │  │ │claude CLI  │ │  │ │claude CLI  │ │          │  │ │
│  │  │  │ │(subprocess)│ │  │ │(subprocess)│ │          │  │ │
│  │  │  │ │stream-json │ │  │ │stream-json │ │          │  │ │
│  │  │  │ └────────────┘ │  │ └────────────┘ │          │  │ │
│  │  │  └────────────────┘  └────────────────┘          │  │ │
│  │  └──────────────────────────────────────────────────┘  │ │
│  │  ┌──────────────────────────────────────────────────┐  │ │
│  │  │         BrowserSessionManager (Playwright)        │  │ │
│  │  │    ブラウザ操作 + CDP Screencast ストリーミング   │  │ │
│  │  └──────────────────────────────────────────────────┘  │ │
│  │  ┌──────────────────────────────────────────────────┐  │ │
│  │  │      MCP Server (Permission + Browser Tools)      │  │ │
│  │  │    permission_prompt / browser_* ◄──► WebSocket   │  │ │
│  │  └──────────────────────────────────────────────────┘  │ │
│  └────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
                              │ WebSocket
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                      Browser (React)                         │
│  ┌──────────────┐  ┌──────────────────────────────────────┐ │
│  │   Sidebar    │  │            Main Area                  │ │
│  │  ┌────────┐  │  │  ┌────────────────────────────────┐  │ │
│  │  │Session1│  │  │  │   MessageStream / BrowserView  │  │ │
│  │  ├────────┤  │  │  │  ┌──────────────────────────┐  │  │ │
│  │  │Session2│  │  │  │  │ [Text Output]            │  │  │ │
│  │  ├────────┤  │  │  │  ├──────────────────────────┤  │  │ │
│  │  │  [+]   │  │  │  │  │ [Permission Request UI]  │  │  │ │
│  │  └────────┘  │  │  │  │  Allow / Deny / Modify   │  │  │ │
│  │              │  │  │  ├──────────────────────────┤  │  │ │
│  │              │  │  │  │ [BrowserView]            │  │  │ │
│  │              │  │  │  │  Canvas + URL bar        │  │  │ │
│  │              │  │  │  └──────────────────────────┘  │  │ │
│  │              │  │  └────────────────────────────────┘  │ │
│  │              │  │  ┌────────────────────────────────┐  │ │
│  │              │  │  │        Input Area              │  │ │
│  │              │  │  └────────────────────────────────┘  │ │
│  └──────────────┘  └──────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
```

### データフロー

1. **ユーザー入力**
   - フロントエンド → WebSocket → サーバー
   - サーバー: `claude -p "prompt" --resume <session_id> --output-format stream-json --permission-prompt-tool mcp__bridge__permission_prompt`

2. **Claude 出力**
   - claude CLI (stream-json) → サーバーでパース → WebSocket → フロントエンド UI

3. **パーミッション要求**
   - claude CLI → MCP Server (permission_prompt ツール呼び出し)
   - MCP Server → WebSocket → フロントエンド UI
   - ユーザー応答 → WebSocket → MCP Server → allow/deny を返す
   - claude CLI が処理を継続

4. **AskUserQuestion**
   - AskUserQuestion は `--allowedTools` に含めることで自動許可
   - または MCP Server で特別処理して WebSocket でフロントエンドに転送

5. **ブラウザ画面ストリーミング**
   - BrowserSessionManager → CDP Screencast → Base64 JPEG → WebSocket → BrowserView (Canvas)
   - ユーザー操作 (click/key/scroll) → WebSocket → Playwright page 操作

---

## プロジェクト構成

```
agent-dock/
├── package.json              # Monorepo root
├── pnpm-workspace.yaml       # pnpm workspaces 設定
├── mcp-config.json           # MCP サーバー設定
├── packages/
│   ├── server/               # バックエンド
│   │   ├── src/
│   │   │   ├── index.ts      # エントリポイント
│   │   │   ├── server.ts     # Hono + WebSocket
│   │   │   ├── session-manager.ts  # セッション管理
│   │   │   ├── claude-runner.ts    # claude CLI 実行
│   │   │   ├── runner-manager.ts   # Runner インスタンス管理
│   │   │   ├── stream-parser.ts    # stream-json パーサー
│   │   │   └── browser-session-manager.ts  # ブラウザセッション管理
│   │   ├── package.json
│   │   └── tsconfig.json
│   │
│   ├── mcp-server/           # MCP サーバー（パーミッション + ブラウザ）
│   │   ├── src/
│   │   │   ├── index.ts      # MCP サーバーエントリポイント
│   │   │   ├── permission-handler.ts  # permission_prompt ツール
│   │   │   └── playwright-handler.ts  # browser_* ツール
│   │   ├── package.json
│   │   └── tsconfig.json
│   │
│   ├── client/               # フロントエンド
│   │   ├── src/
│   │   │   ├── main.tsx
│   │   │   ├── App.tsx
│   │   │   ├── components/
│   │   │   │   ├── Sidebar.tsx
│   │   │   │   ├── SessionTab.tsx
│   │   │   │   ├── MessageStream.tsx     # メッセージ表示
│   │   │   │   ├── PermissionRequest.tsx # パーミッション UI
│   │   │   │   ├── AskUserQuestion.tsx   # 質問 UI
│   │   │   │   ├── DiffView.tsx          # 差分表示
│   │   │   │   ├── InputArea.tsx         # 入力エリア
│   │   │   │   └── BrowserView.tsx       # ブラウザビュー
│   │   │   ├── hooks/
│   │   │   │   ├── useWebSocket.ts
│   │   │   │   └── useSession.ts
│   │   │   └── styles/
│   │   ├── package.json
│   │   └── vite.config.ts
│   │
│   └── shared/               # 共有型定義
│       ├── src/
│       │   └── index.ts      # WebSocket メッセージ型
│       └── package.json
│
├── DESIGN.md                 # この設計ドキュメント
└── README.md
```

---

## 技術選定

| レイヤー | 技術 | バージョン |
|---------|------|-----------|
| ランタイム | Node.js | 22.x / 24.x |
| 言語 | TypeScript | 5.9.x |
| フロントエンド | React | 19.2.x |
| UI コンポーネント | shadcn/ui | (Tailwind v4対応) |
| CSS | Tailwind CSS | 4.x |
| 差分表示 | react-diff-viewer-continued | 3.4.x |
| 通信 | ws | 8.18.x |
| バックエンド | Hono | 4.11.x |
| Claude 実行 | `claude` CLI | (subprocess, stream-json) |
| MCP Server | @modelcontextprotocol/sdk | 1.12.x |
| ビルド (client) | Vite | 7.x |
| ビルド (server) | tsc | - |
| モノレポ | pnpm workspaces | - |
| テスト | vitest | 3.x |
| ブラウザ自動化 | Playwright | (via @anthropic/playwright-mcp) |
| 画面ストリーミング | CDP Screencast | (via @anthropic/browser-streamer) |

---

## WebSocket プロトコル

### メッセージ形式

```typescript
// クライアント → サーバー
type ClientMessage =
  // セッション管理
  | { type: 'create_session'; name?: string; workingDir?: string }
  | { type: 'attach_session'; sessionId: string }
  | { type: 'detach_session'; sessionId: string }
  | { type: 'delete_session'; sessionId: string }
  | { type: 'rename_session'; sessionId: string; name: string }
  | { type: 'list_sessions' }
  // ユーザー入力
  | { type: 'user_message'; sessionId: string; content: string }
  | { type: 'interrupt'; sessionId: string }
  // パーミッション応答
  | { type: 'permission_response'; sessionId: string; requestId: string;
      response: { behavior: 'allow'; updatedInput: any } | { behavior: 'deny'; message: string } }
  // AskUserQuestion 応答
  | { type: 'question_response'; sessionId: string; requestId: string;
      answers: Record<string, string> }
  // ブラウザ操作
  | { type: 'start_screencast'; sessionId: string }
  | { type: 'stop_screencast'; sessionId: string }
  | { type: 'user_browser_click'; sessionId: string; x: number; y: number }
  | { type: 'user_browser_key_press'; sessionId: string; key: string }
  | { type: 'user_browser_scroll'; sessionId: string; deltaX: number; deltaY: number }
  | { type: 'user_browser_navigate'; sessionId: string; url: string }
  | { type: 'user_browser_back'; sessionId: string }
  | { type: 'user_browser_forward'; sessionId: string }
  | { type: 'user_browser_refresh'; sessionId: string };

// サーバー → クライアント
type ServerMessage =
  // セッション管理
  | { type: 'session_created'; session: SessionInfo }
  | { type: 'session_attached'; sessionId: string; history: MessageItem[] }
  | { type: 'session_list'; sessions: SessionInfo[] }
  | { type: 'session_deleted'; sessionId: string }
  // Agent 出力
  | { type: 'text_output'; sessionId: string; text: string }
  | { type: 'tool_use'; sessionId: string; toolName: string; toolUseId: string; input: any }
  | { type: 'tool_result'; sessionId: string; toolUseId: string; content: string; isError?: boolean }
  | { type: 'result'; sessionId: string; result: string }
  // パーミッションリクエスト
  | { type: 'permission_request'; sessionId: string; requestId: string;
      toolName: string; input: any }
  // AskUserQuestion
  | { type: 'ask_user_question'; sessionId: string; requestId: string;
      questions: Array<{ question: string; header: string;
                         options: Array<{ label: string; description: string }>;
                         multiSelect: boolean }> }
  // ブラウザ画面
  | { type: 'screencast_frame'; sessionId: string; data: string; metadata: ScreencastMetadata }
  | { type: 'screencast_status'; sessionId: string; active: boolean; browserUrl?: string; browserTitle?: string }
  | { type: 'screencast_cursor'; sessionId: string; cursor: string }
  // エラー
  | { type: 'error'; sessionId?: string; message: string };

interface ScreencastMetadata {
  deviceWidth: number;
  deviceHeight: number;
  timestamp: number;
}
```

### セッション情報

```typescript
interface SessionInfo {
  id: string;              // Bridge のセッション ID
  name: string;            // 表示名
  createdAt: string;       // 作成日時
  workingDir: string;      // 作業ディレクトリ
  status: SessionStatus;   // 状態
  claudeSessionId?: string; // Claude CLI のセッション ID (--resume 用)
}

type SessionStatus = 'running' | 'waiting_input' | 'waiting_permission' | 'idle';
```

---

## 主要コンポーネント

### ClaudeRunner (packages/server/src/claude-runner.ts)

Claude CLI を subprocess として実行し、stream-json 出力をパースするクラス。

**責務:**
- `claude` CLI の起動 (`spawn`)
- stream-json (NDJSON) 出力のパース
- イベントの発行 (`text`, `tool_use`, `tool_result`, `result`, `system`, `error`, `exit`)

**オプション:**
- `workingDir`: 作業ディレクトリ
- `claudePath`: claude コマンドのパス
- `mcpConfigPath`: MCP 設定ファイルのパス
- `permissionToolName`: パーミッションツール名

### RunnerManager (packages/server/src/runner-manager.ts)

セッションごとの ClaudeRunner インスタンスを管理するクラス。

**責務:**
- ClaudeRunner インスタンスのライフサイクル管理
- セッション ID と Runner の紐付け
- イベントの転送

### SessionManager (packages/server/src/session-manager.ts)

セッションのメタデータを管理するクラス。

**責務:**
- セッションの作成/削除/更新
- メッセージ履歴の保持
- Claude セッション ID の紐付け

### StreamJsonParser (packages/server/src/stream-parser.ts)

Claude CLI の stream-json 出力をパースするユーティリティ。

**対応イベント:**
- `assistant`: テキスト出力、ツール使用
- `user`: ツール結果
- `result`: 処理完了
- `system`: システムメッセージ

### BrowserSessionManager (packages/server/src/browser-session-manager.ts)

セッションごとにブラウザインスタンスを管理し、CDP Screencast による画面ストリーミングを提供。

**責務:**
- セッションごとのブラウザライフサイクル管理
- CDP Screencast でフレーム取得・転送
- `framenavigated` イベントで URL/タイトル同期

**イベント:** `frame`, `status`, `error`

### BrowserView (packages/client/src/components/BrowserView.tsx)

ブラウザ画面ストリームを Canvas で表示し、ユーザー操作を処理するコンポーネント。

**機能:**
- Canvas によるフレーム描画（レスポンシブ、アスペクト比維持）
- ブラウザ chrome UI（タイトルバー、ナビゲーションバー）
- 編集可能な URL バー、戻る/進む/リロードボタン
- マウス/キーボード操作のキャプチャ、カーソル同期

---

## 実装ステップ

### Phase 1: プロジェクト基盤 ✅
1. **プロジェクト初期化** ✅
   - pnpm workspaces でモノレポ構成
   - TypeScript 設定
   - shared パッケージで型定義を共有

2. **バックエンド基盤** ✅
   - Hono サーバー
   - WebSocket サーバー (ws)
   - 基本的な接続確認

3. **フロントエンド基盤** ✅
   - Vite + React + TypeScript
   - WebSocket クライアント (useWebSocket hook)

### Phase 2: Claude CLI 統合 🚧
4. **Claude CLI 実行基盤** ✅
   - child_process で claude CLI を実行
   - stream-json 出力をパース
   - NDJSON ストリーム処理

5. **MCP Server 実装** 🚧
   - @modelcontextprotocol/sdk でサーバー作成
   - `permission_prompt` ツールを実装
   - WebSocket でバックエンドと通信

6. **フロントエンド UI コンポーネント** ⏳
   - MessageStream: テキスト出力表示
   - PermissionRequest: パーミッション許可 UI
   - InputArea: ユーザー入力

### Phase 3: セッション管理 ⏳
7. **セッション管理**
   - `--resume <session_id>` でセッション継続
   - メッセージ履歴の保持

8. **サイドバー UI**
   - セッション一覧表示
   - セッション作成/削除/名前変更

### Phase 4: 仕上げ ⏳
9. **AskUserQuestion UI**
   - Claude からの質問を選択肢 UI で表示

10. **差分表示 UI**
    - Write/Edit ツールの差分をビジュアル表示
    - react-diff-viewer-continued で実装

11. **UX改善**
    - エラーハンドリング
    - ローディング状態
    - 中断機能（SIGINT 送信）

---

## 検証方法

### ローカル開発環境でのテスト

```bash
# 依存関係インストール
pnpm install

# サーバー起動
cd packages/server && pnpm dev

# クライアント起動（別ターミナル）
cd packages/client && pnpm dev
```

http://localhost:5173 でアクセス

### テスト実行

```bash
# 全パッケージのテスト
pnpm -r test

# 特定パッケージのテスト
cd packages/server && pnpm test
```

### 動作確認チェックリスト

- [ ] セッション作成ができる
- [ ] ユーザーメッセージを送信できる
- [ ] Claude からの応答がリアルタイム表示される（stream-json）
- [ ] パーミッションリクエストが UI に表示される（MCP 経由）
- [ ] パーミッションを許可/拒否できる
- [ ] AskUserQuestion が選択肢 UI で表示される
- [ ] 選択後に Claude が処理を継続する
- [ ] 複数セッションを切り替えられる（--resume）
- [ ] ブラウザを閉じて再接続しても履歴が表示される
- [ ] ブラウザ画面がリアルタイムでストリーミングされる
- [ ] URL バーに現在の URL/タイトルが表示される
- [ ] URL バーから直接ナビゲーションできる
- [ ] 戻る/進む/リロードボタンが機能する
- [ ] マウスクリック・キーボード入力がブラウザに反映される

---

## 注意事項

### MCP Server の制約

- MCP ツールは **同期的にレスポンスを返す必要がない**（Promise を返せる）
- ユーザー入力を待機する間、Promise を resolve せずに待機可能
- ただし、タイムアウト（60秒）に注意

### stream-json 出力形式

- NDJSON (Newline Delimited JSON) 形式
- 各行が独立した JSON オブジェクト
- `type` フィールドでメッセージ種別を判別

### Claude Code セッションの永続性

- セッションは `~/.claude/sessions/` に自動保存される
- `--resume <session_id>` で再開可能
- サーバー側ではセッション ID のマッピングのみ管理

### Claude Max ライセンス

- `ANTHROPIC_API_KEY` 環境変数が **設定されていない** ことを確認
- Claude Max subscription で実行されていることを確認
- Anthropic が公式に許可している方法のみ使用

---

## 設計パターン

### Per-Session State Management

複数セッションを同時に扱う場合、グローバル state ではなく **Map でセッションごとに状態を分離**する。

**問題**: 単一の state で管理すると、セッション切り替え時に状態が混乱する。
- 例: Session A の PermissionRequest が Session B を見ているときに表示される

**解決策**: Map<sessionId, State> でセッションごとに分離

```typescript
// Bad: 単一の state（セッション切り替え時に混乱）
const [pendingQuestion, setPendingQuestion] = useState(null);

// Good: セッションごとに分離
const [sessionPendingQuestion, setSessionPendingQuestion] = useState<Map<string, PendingQuestion>>(new Map());

// 現在のセッションの state を computed value として取得
const pendingQuestion = activeSessionId
  ? sessionPendingQuestion.get(activeSessionId)
  : null;
```

**適用箇所**:
- `pendingPermission`: パーミッション要求
- `pendingQuestion`: AskUserQuestion 要求
- `sessionMessages`: メッセージ履歴
- `sessionUsageInfo`: トークン使用量

### Real-time Status Broadcast

セッション状態の変更（idle → running など）は、**全クライアントにブロードキャスト**する。

**理由**: 複数ブラウザタブ/ウィンドウからアクセスした場合に状態を同期するため。

```typescript
// サーバー側: 状態変更時に全クライアントへ通知
function broadcastStatusChange(sessionId: string, status: SessionStatus) {
  broadcastToAll({
    type: 'session_status_changed',
    sessionId,
    status,
  });
}
```

**注意**: `session_attached` のような特定クライアント向けメッセージと異なり、ステータス変更は全クライアントが知る必要がある。

### Defensive Rendering for External Data

DB やネットワークから取得したデータを表示するコンポーネントは、**Guard clause で不正データを防御**する。

**問題**: 古いDBデータや予期せぬ形式のデータでコンポーネントがクラッシュする可能性がある。

**解決策**: コンポーネント冒頭で Guard clause を入れ、フォールバック UI を表示

```typescript
function QuestionMessage({ content }: { content: QuestionMessageContent }) {
  // Guard against invalid content
  if (!content || !Array.isArray(content.answers)) {
    return (
      <div data-testid="message-item">
        <span>AskUserQuestion</span>
        <span>User answered questions.</span>
      </div>
    );
  }

  // Normal rendering
  return content.answers.map(...);
}
```

**テスト要件**: 外部データを扱うコンポーネントには堅牢性テストを追加する

```typescript
describe('QuestionMessage robustness', () => {
  it('should not crash with null content', () => { ... });
  it('should not crash with undefined content', () => { ... });
  it('should not crash with legacy data structure', () => { ... });
});
```

### AgentDock 統合 MCP ツールの自動許可

AgentDock が提供する MCP ツール（`mcp__bridge__*`）は、**ユーザーの許可なしに自動実行**される。

**理由**:
- AgentDock 統合 MCP ツールは AgentDock 自身が提供し、UI と密接に連携するため信頼できる
- ブラウザ操作、ポート監視など、頻繁に実行されるツールで毎回許可を求めるのは UX が悪い
- 外部 MCP ツール（`mcp__plugin_*`）とは明確に区別される

**自動許可されるツール**:
1. **AgentDock 統合 MCP ツール** (`mcp__bridge__*`)
   - `mcp__bridge__browser_*`: ブラウザ操作ツール
   - `mcp__bridge__port_monitor`: ポート監視ツール
   - `mcp__bridge__permission_prompt`: 内部使用（パーミッション処理）
   - 今後追加される `mcp__bridge__*` ツールも自動的に許可される

2. **AskUserQuestion**: UI インタラクションツール（ユーザー操作が必須なためセキュリティリスクなし）

**自動許可されないツール**:
- **外部 MCP ツール** (`mcp__plugin_*`): 外部 MCP サーバーが提供するツール
- **Claude Code 標準ツール** (`Bash`, `Write`, `Edit` など): ファイル操作やコマンド実行は許可が必要

**実装**:
```typescript
// packages/server/src/server.ts

// AgentDock 統合 MCP ツールの判定
function isAgentDockTool(toolName: string): boolean {
  return /^mcp__bridge__/.test(toolName);
}

// 自動許可ツールの判定
function isAutoAllowedTool(toolName: string): boolean {
  if (isAgentDockTool(toolName)) return true;
  if (toolName === 'AskUserQuestion') return true;
  return false;
}
```

**セキュリティ考慮**:
- AgentDock 統合ツールは、AgentDock 開発者が安全性を保証するものに限定
- 新しいツールを追加する際は、自動許可が適切かどうかを慎重に検討すること
- 外部 MCP ツールは絶対に自動許可リストに含めない

### リポジトリ登録機能

よく使うディレクトリや Git リポジトリを事前登録し、セッション開始時に選択できる機能。リポジトリを選択してセッションを開始すると、複数セッションの同時並行開発が可能になる。

#### リポジトリタイプ

```
Repository Type
├── local              # ローカルディレクトリ（非 Git）
├── local-git-worktree # ローカル Git リポジトリ（worktree 使用）
└── remote-git         # リモート Git リポジトリ
    ├── github         # GitHub (provider)
    ├── gitlab         # GitLab (provider) - 将来
    └── ...            # その他 (provider) - 将来
```

| タイプ | 説明 | セッション開始時の動作（Native） | Container モード |
|--------|------|----------------------------------|------------------|
| `local` | 任意のディレクトリ | tmpfs にコピー | そのまま使用（CoW） |
| `local-git-worktree` | ローカル Git リポジトリ | `.worktree/agentdock-{sessionId}` に worktree 作成 | そのまま使用（CoW） |
| `remote-git` | リモート Git リポジトリ | cache に clone/fetch → worktree 作成 | そのまま使用（CoW） |

#### ディレクトリ構造

```
{serverRoot}/
└── cache/                           # キャッシュディレクトリ (gitignore)
    └── repos/                       # リポジトリキャッシュ
        └── {repoId}/                # remote-git: リモートからclone
            └── .worktree/           # worktree はここに作成
                └── agentdock-{sessionId}/

/tmp/agent-dock-repos/               # tmpfs ベースパス（設定可能）
└── {sessionId}/                     # local タイプのコピー先
```

#### WebSocket メッセージ

```typescript
// クライアント → サーバー
| { type: 'list_repositories' }
| { type: 'create_repository'; name: string; path: string; repositoryType: RepositoryType; remoteUrl?: string; remoteBranch?: string }
| { type: 'update_repository'; id: string; name?: string; path?: string; repositoryType?: RepositoryType }
| { type: 'delete_repository'; id: string }

// サーバー → クライアント
| { type: 'repository_list'; repositories: Repository[] }
| { type: 'repository_created'; repository: Repository }
| { type: 'repository_updated'; repository: Repository }
| { type: 'repository_deleted'; id: string }
```

#### create_session 拡張

```typescript
interface CreateSessionMessage {
  type: 'create_session';
  name?: string;
  workingDir?: string;
  runnerBackend?: RunnerBackend;
  browserInContainer?: boolean;
  // リポジトリ選択
  repositoryId?: string;      // workingDir の代わりにリポジトリを指定
  worktreeName?: string;      // Git リポジトリ用カスタム worktree 名
}
```

#### 設定

| 設定 | デフォルト | 説明 |
|------|-----------|------|
| `tmpfsBasePath` | `/tmp/agent-dock-repos/` | local タイプのコピー先 |
| `cacheDir` | `{serverRoot}/cache/` | remote-git の clone 先 |
